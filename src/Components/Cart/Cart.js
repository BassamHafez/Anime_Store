...
const Cart = () => {
  ...
  const ctxProducts = cartCtx.products.map((product) => {
    return (
      <CartItem
        onDelete={onDeleteHandler.bind(null, product.id)}
        onAdd={onAddHandler.bind(null, product)}
        onRemove={onRemoveHandler.bind(null, product.id)}
        key={product.id}
        product={product}
      />
    );
  ...
  return (
    <Fragment>
      <div
        className={`${style.off_canvas} offcanvas offcanvas-start`}
        tabIndex="-1"
        id="offcanvasExample"
      ...
      >
        ...
        <div className="offcanvas-body py-4">
          <ul className="px-0">{ctxProducts}</ul>
        </div>

        <div
          className={`${style.offcanvas_footer}  offcanvas-footer d-flex justify-content-evenly align-items-center`}
        >
